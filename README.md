...
| File (горячие) | AWS S3 хранилище | Надежное, удобное хранение для файлов с быстрым доступом | По хэшу от почты + дате | Резервное копирование через AWS Backup |
| File (холодные)	| S3 Glacier Deep Archive | Архивы (старше 3 мес.) | По году | 3 копии |
...
|---------------------|---------------------------|------------------|
| **Go**              | Бэкенд-сервисы           | Высокая производительность, отличная поддержка многопоточности, идеально подходит для микросервисной архитектуры |
| **gRPC** | Способ общения микросервисов	| Общение между различными сервисами |
| **Cassandra**       | Основное хранилище писем | Горизонтальная масштабируемость, высокая доступность, оптимизирована для записи |
| **AWS S3**          | Хранение вложений        | Надежное объектное хранилище с высокой доступностью и масштабируемостью |
| **S3 Glacier Deep Archive** | Хранение холодных вложений | Долгосрочное архивирование данных |
...
| **Redis**           | Кэширование             | Идеально для хранения сессий и горячих данных, сверхнизкие задержки |
...
| **ClickHouse**      | Аналитика               | Высокая скорость выполнения аналитических запросов |
| **Prometheus**      | Мониторинг              | Сбор и хранение метрик в режиме реального времени |
| **Grafana**         | Визуализация метрик    | Гибкие дашборды для анализа производительности |
...
| AWS S3 (Горячие файлы) | Multi-AZ репликация для защиты от потерь и Cross-Region Replication (CRR) для аварийного восстановления |
| S3 Glacier (Холодные файлы) | 3 копии в разных регионах. Архивация данных старше 3 месяцев. Шардирование по году. |
| Redis | RDB для создания снимков данных с определённым интервалом. Репликация master-slave для отказоустойчивости. |
Особенности реализации:
- Для PostgreSQL: синхронная репликация гарантирует отсутствие потерь данных
- Для Cassandra: данные дублируются в географически распределённых дата-центрах
- Для S3: автоматическое реплицирование между bucket'ами с versioning
- Для Glacier: долгосрочное хранение с гарантией сохранности
- Для Redis: комбинация RDB и AOF для максимальной сохранности данных
## Список источников
